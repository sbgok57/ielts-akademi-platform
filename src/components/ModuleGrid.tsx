    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {MODULLER.map((m) => (
        <Link
          key={m.yol}
          href={m.yol}
          className="flex items-start gap-3 rounded-3xl border p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          style={{
            borderColor: "var(--border)",
            background: "var(--bg-soft)",
          }}
        >
          <img src={m.ikon} alt="" width={44} height={44} className="rounded-xl" loading="lazy" />
          <span>
            <strong
